...
Londonborough <- st_read(here::here("prac_5",
                                    "Data",
...
Worldcities <- st_read(here::here("prac_5",
                                  "Data",
                                  "World_Cities", 
                                  "World_Cities.shp")) %>%
  st_transform(., 27700)
...
```{r}
UK_outline <- st_read(here::here("prac_5", 
                                 "Data",
                                 "gadm41_GBR.gpkg")) %>%
  st_transform(., 27700)
...
Airbnb <- read_csv(here::here("prac_5", "Data", "listings.csv")) %>%
...
UK_outlinecrop <- UK_outline$geom %>%
  st_crop(., newbb)
...
```{r}
tmap_save(t, here::here("prac_5", "Map", "hotelsandairbnbR.png"))

library(grid)
tmap_save(main,insets_tm = inset,insets_vp=viewport(x=0.86, y=0.29, width=.5, height=.55), filename=here::here("prac_5", "Map", "test.pdf"), dpi=600)
```
